...
export default function Lobby({ onNavigate, playMode, onModeSelect }) {
  ...
  const [onlineSelectingDeck, setOnlineSelectingDeck] = useState(false);
  ...
  // Load profile decks for authenticated players (used in online deck selection)
  useEffect(() => {
    if (!currentUser || !supabase) {
    ...
    supabase.from('decks').select('*').order('created_at', { ascending: false })
      .then(({ data }) => setProfileDecks(data ?? []));
  }, [currentUser]);
  ...
  async function handleCreateGame(deckId) {
    if (!supabase) {
    ...
    setCreating(true);
    setCreateError(null);
    setOnlineSelectingDeck(false);

    const guestId = getGuestId();
    const gameId = generateGameId();

    // Placeholder state satisfies the game_state NOT NULL constraint.
    // The real state is generated by useMultiplayerGame once both players
    // have confirmed their decks during the deck_select phase.
    ...
    // Parse custom JSON deck specs into objects for JSONB storage;
    // plain faction IDs (e.g. 'human') are stored as-is.
    let deckSpecJson;
    if (typeof deckId === 'string' && deckId.startsWith('{')) {
      try { deckSpecJson = JSON.parse(deckId); } catch { deckSpecJson = deckId; }
    } else {
      deckSpecJson = deckId ?? null;
    }
    ...
    const { error } = await supabase.from('game_sessions').insert({
      ...
      status: 'waiting',
      player1_deck: deckId ?? null,
      host_deck: deckSpecJson,
      player2_deck: null,
    });
    ...
    if (error) {
      console.error('[CreateGame] Supabase insert error:', JSON.stringify(error, null, 2));
  ...
  // Deck selection step before creating an online game: player picks their
  // faction here so the host_deck is stored in the session from the start.
  if (onlineSelectingDeck) {
    return (
      <div style={{ position: 'relative' }}>
        <DeckSelect
          onSelect={(deckId) => handleCreateGame(deckId)}
        ...
        <button
          ...
          onClick={() => setOnlineSelectingDeck(false)}
        >
          ← Back
        </button>
      </div>
        ...
        {!playMode ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <button
              ...
              onClick={() => setOnlineSelectingDeck(true)}
              disabled={creating}
            >
